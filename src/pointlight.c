...
// ITS ALL FUCKED DONT EVEN BOTHER
...
void ex_point_light_init()
{
  ...
  // mat4x4_perspective(point_shadow_projection, rad(90.0f), aspect, 1.0f, EX_POINT_FAR_PLANE);
}
...
void ex_point_light_begin(ex_point_light_t *l, int face)
{
  ...
  mat4x4_look_at(l->transform[2], l->position, temp, (vec3){0.0f, -1.0f, 0.0f});
  mat4x4_mul(l->transform[2], point_shadow_projection, l->transform[2]);

  vec3_add(temp, l->position, (vec3){0.0f, -1.0f, 0.0f});
  mat4x4_look_at(l->transform[3], l->position, temp, (vec3){0.0f, -1.0f, 0.0f});
  ...
  glUniform1i(glGetUniformLocation(l->shader, "u_face"), face);
